...
local function getPositions()
	...
	elseif Game == "Super Mario Bros." then
		...
		marioState = memory:read_u8(0x000E)
		if marioState ~= marioStatePrev then
			marioStatePrev = marioState
			emu.print_error("MarioState " .. marioState)
		end
...
local function clearJoypad()
	...
	for b = 1,#ButtonNames do
		for name, port in pairs(manager.machine.ioport.ports) do
			if string.match(name,"ctrl1") then
				for fname, field in pairs(port.fields) do
					--if fname == ButtonNames[b] then
					if field.name == ButtonNames[b] then
						local button = {}
						button.port = port
						button.field = field
						button.state = 0
...
local function displayGenome(genome)
	...
	gui:draw_box(50-BoxRadius*5-3,70-BoxRadius*5-3,50+BoxRadius*5+2,70+BoxRadius*5+2, 0x80808080,0x80000000)
...
local function isEndOfRunCriteriaMet()
	local isPerGameCriteriaMet = false
	if Game == "Super Mario Bros." then
		-- Detect Mario is falling
		if marioY > 259 then
			isPerGameCriteriaMet = true
		end
		-- Mario is dying
	...
	return timeoutReached or isPerGameCriteriaMet
end
...
--	end
	...
	if isEndOfRunCriteriaMet() == true then
